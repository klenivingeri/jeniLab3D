...
export function mountAcompanhamentoPage(container, { onSalvoHistorico, onAbrirPedido } = {}) {
    ...
    function cardPedido(p) {
        return `
            <div data-abrir-pedido="${p.id}" class="bg-darkbg border border-gray-700/60 rounded-xl p-3.5 space-y-2 shadow cursor-pointer hover:border-accent/50 transition">
                <div class="flex justify-between items-start">
                    <h4 class="text-white font-semibold text-sm">${p.nome}</h4>
                    <button data-remover-pedido="${p.id}" class="text-gray-500 hover:text-red-400 text-xs"><i class="fa-solid fa-trash"></i></button>
                </div>
                ...
                <div class="flex justify-between items-center pt-2 border-t border-gray-800 text-xs">
                    ...
                    <select data-status-pedido="${p.id}" class="bg-gray-800 border border-gray-700 text-white rounded px-2 py-1 text-xs focus:outline-none">
                ...
                ${
                    p.status === 'Enviado'
                        ? `<button data-salvar-pedido="${p.id}" class="w-full px-3 py-1.5 bg-accent/10 border border-accent/30 text-accent rounded-lg text-xs font-semibold hover:bg-accent/20 transition flex items-center justify-center space-x-2"><i class="fa-solid fa-floppy-disk"></i><span>Salvar no Histórico</span></button>`
                        : ''
                }
            </div>
        `;
        ...
        listaEl.innerHTML = COLUNAS.map((col) => {
            ...
            return `
                <div class="bg-darkcard border border-gray-800 rounded-2xl p-4 flex flex-col h-[600px]">
                    ...
                    <div class="overflow-y-auto flex-grow space-y-3 pr-1">
                        ${
                            pedidosCol.length === 0
                                ? '<div class="text-center py-12 text-xs text-gray-600">Nenhum pedido</div>'
